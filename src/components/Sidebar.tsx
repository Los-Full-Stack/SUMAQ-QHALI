    ...
    <aside className="w-full lg:w-56 bg-primary text-white flex flex-col border-b lg:border-b-0 lg:border-r border-[#042A46] shrink-0 lg:h-screen overflow-y-auto beautiful-scrollbar relative z-20">
      ...
      <div className="p-6 border-b border-[#042A46] flex items-center justify-between">
        ...
        <button 
          ...
          className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors cursor-pointer ${isOffline ? "bg-rose-50/75 text-white" : "bg-[#0b3c64] text-cyan-300"}`}
        >
      ...
      <div className="p-6 border-b border-[#042A46] bg-[#002f54] flex items-center gap-3">
        ...
        <div className="font-sans text-white">
          ...
          <p className="text-[10px] text-cyan-200">
            {role === "patient_portal" ? (language === "es" ? "Paciente Registrado" : "Qillqasqa Paciente") : (language === "es" ? "Director Regional Urubamba" : "Urubamba Suyu Kamachiq")}
          </p>
        </div>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col gap-6 font-sans">
        <div className="flex flex-col gap-1.5">
          <p className="text-[10px] text-cyan-300 uppercase tracking-widest font-semibold px-2.5 mb-1">
            {language === "es" ? "Módulos" : "T'aqakuna"}
          </p>
          {role === "doctor" && (
            <button 
            ...
            >
              <span className="flex items-center gap-2"><LayoutDashboard className="w-4 h-4 shrink-0" />{language === "es" ? "Panel Médico" : "Hampiqpa Panelnin"}</span>
            </button>
          )}
          ...
          {role === "administrator" && (
            <>
              <button 
                ...
                className={`w-full flex items-center justify-between text-left px-3 py-2.5 rounded-lg text-xs md:text-sm font-semibold transition-all cursor-pointer bg-white text-primary shadow-sm`}
              >
                <span className="flex items-center gap-2"><Building2 className="w-4 h-4 shrink-0" />{language === "es" ? "Panel Admin" : "Kamachiypa Panelnin"}</span>
              </button>
              <button 
                onClick={() => handleNavigation("doctor", "/doctor")}
        ...
        <div className="mt-auto flex flex-col gap-2">
          ...
          >
            <span className="flex items-center gap-1.5"><Globe className="w-3.5 h-3.5" />{language === "es" ? "Idioma:" : "Simi:"}</span>
          ...
          {role === "doctor" && (
            ...
            >
              <UserPlus className="w-4 h-4" /> {language === "es" ? "Registrar Paciente" : "Pacienteta qillqay"}
            </button>
          )}
          ...
          >
            <LogOut className="w-4 h-4 text-rose-400" /> {language === "es" ? "Salir" : "Lluqsiy"}
          </button>
        </div>
      </div>
    </aside>
