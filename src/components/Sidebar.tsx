    ...
    <aside className="w-full lg:w-56 bg-primary text-white flex flex-col border-b lg:border-b-0 lg:border-r border-[#042A46] shrink-0 lg:h-screen overflow-y-auto beautiful-scrollbar relative z-20">
      ...
      <div className="p-6 border-b border-[#042A46] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-teal-200 uppercase tracking-widest font-semibold">Bilingual EHR</p>
          </div>
        </div>
        <button 
          onClick={() => setIsOffline(!isOffline)}
          className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors cursor-pointer ${isOffline ? "bg-rose-500 text-white" : "bg-[#0b3c64] text-teal-300"}`}
        >
      ...
      <div className="p-6 border-b border-[#042A46] bg-[#002f54] flex items-center gap-3">
        <div className="relative">
          {role === "patient_portal" && portalPatient ? (
            <div className="w-12 h-12 rounded-full border-2 border-slate-50 bg-teal-600 flex items-center justify-center text-white font-black text-xl shadow-md">
          ...
          <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 rounded-full border-2 border-primary"></span>
        </div>
      ...
      <div className="p-4 flex-1 flex flex-col gap-6 font-sans">
        <div className="flex flex-col gap-1.5">
          <p className="text-[10px] text-teal-300 uppercase tracking-widest font-semibold px-2.5 mb-1">Módulos</p>
          {role === "doctor" && (
            <button 
        ...
        <div className="mt-auto flex flex-col gap-2">
          <button 
            ...
            className="w-full bg-[#002b4d] border border-[#042844] hover:bg-[#00213d] flex items-center justify-between px-3 py-2 rounded-lg text-xs text-teal-100 font-semibold cursor-pointer"
          >
