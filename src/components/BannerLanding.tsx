    ...
    <div className="flex-grow flex flex-col bg-[#F8FAFC]">
      <nav className="sticky top-0 z-40 flex justify-between items-center w-full px-4 md:px-10 py-3 bg-white shadow-sm border-b border-gray-200">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 rounded-lg bg-emerald-600 text-white flex items-center justify-center p-1.5 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all cursor-pointer shadow-md shadow-emerald-600/20"
          >
      ...
      <section className="relative w-full overflow-hidden bg-white border-b border-gray-100">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 py-16 md:py-24 flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-1 flex flex-col gap-6 z-10">
            <span className="w-fit bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs px-4 py-1.5 rounded-full font-bold shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-4">
              <button 
                onClick={() => setShowAuthModal(true)}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm px-8 py-3.5 rounded-xl shadow-lg shadow-emerald-600/30 transition-all flex items-center justify-center gap-2 hover:-translate-y-0.5 cursor-pointer"
              >
            ...
            <div className="p-8">
              ...
              <div className="flex p-1 bg-slate-100 rounded-xl mb-6">
                <button 
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${authRole === "patient" ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500"}`}
                >
                  <UserCheck className="w-4 h-4" /> Paciente
                </button>
                <button 
                  onClick={() => setAuthRole("staff")}
                  className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${authRole === "staff" ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500"}`}
                >
              ...
              <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
                {authMode === "register" && authRole === "patient" && (
                  <>
                    <div>
                      ...
                      <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all" placeholder="Ej. Juan Mamani" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-slate-600 ml-1 mb-1 block">Celular</label>
                        <input type="text" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-all" placeholder="Opcional" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-slate-600 ml-1 mb-1 block">Comunidad</label>
                        <select value={location} onChange={e => setLocation(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-all">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all" 
                ...
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm py-3.5 rounded-xl transition-all shadow-md mt-2 disabled:opacity-70 disabled:cursor-not-allowed"
                >
              ...
              {authRole === "patient" && (
                <div className="mt-6 text-center text-xs font-medium text-slate-500">
                  {authMode === "login" ? (
                    <>¿No tienes una cuenta? <button type="button" onClick={() => setAuthMode("register")} className="text-emerald-600 font-bold hover:underline">Regístrate aquí</button></>
                  ) : (
                    <>¿Ya tienes una cuenta? <button type="button" onClick={() => setAuthMode("login")} className="text-emerald-600 font-bold hover:underline">Inicia sesión</button></>
                  )}
                </div>
              )}
            </div>
